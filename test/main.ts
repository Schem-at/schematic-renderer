import { SchematicRenderer } from "../src/index";
...
const adderSchematicBase64String =
	"";
		...
		request.onsuccess = function (event) {
			if (request.result) {
				...
				fetch(jarURL)
					...
					.then((blob) => {
						console.log("Jar fetched from Mojang, storing in IndexedDB...");
						return blob;
					})
					.then((blob) => {
						const addRequest = db
