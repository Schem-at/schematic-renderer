...
export class RessourceLoader {
	...
	): Promise<THREE.Material> | undefined {
	...
	public getColorForElement(
		...
		if (tex.startsWith("block/redstone_dust_")) {
			...
			return this.REDSTONE_COLORS[power as number];
	...
	public async processFaceData(
		...
		for (const face of this.POSSIBLE_FACES) {
			...
			if (!this.materialMap.has(materialId)) {
				...
				const base64Material = await this.getBase64Image(model, faceData);
				this.base64MaterialMap.set(
					materialId,
					base64Material ?? "data:image/png;base64,"
				);
			}
	...
	public occludedFacesIntToList(occludedFaces: number) {
	...
	public getOccludedFacesForBlock(blockType: string, pos: Vector): number {
		const { x, y, z } = pos;
