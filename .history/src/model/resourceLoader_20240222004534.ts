...
export async function getResourceLoader(
	...
	const stringCache: Map<string, string | undefined> = new Map();
	const blobCache: Map<string, string | undefined> = new Map();
		...
		if (blobCache.has(name)) {
			...
			if (Array.isArray(zip)) {
				for (const zipFile of zip) {
					data = await zipFile.file(`assets/minecraft/${name}`)?.async("blob");
					if (data) {
						break;
					}
				}
				...
				data = await zip.file(`assets/minecraft/${name}`)?.async("blob");
			}
			if (!data) {
	...
	const getResourceString = async (name: string) => {
		if (stringCache.has(name)) {
			...
			if (Array.isArray(zip)) {
				...
				data = await zip.file(`assets/minecraft/${name}`)?.async("string");
			}
