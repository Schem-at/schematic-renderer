...
export function getModelLoader(resourceLoader: ResourceLoader): ModelLoader {
	...
	async function getTextureMaterial(
		...
		const blob = await resourceLoader.getResourceBlob(`textures/${tex}.png`);
		if (blob === undefined) {
			return undefined;
		}
	...
	const getModel = async (
		...
		if (modelCache.has(data.name)) {
			return modelCache
				.get(data.name)
				.map((mesh, i) => mesh.createInstance(`instance-${data.name}-${i}`));
		}

		const group: Mesh[] = [];
		for (let modelIndex = 0; modelIndex < data.holders.length; modelIndex++) {
			...
			for (const element of model.elements) {
				...
				const faceData = await processFaceData(element, model, scene, block);
				...
				const multiMat = new MultiMaterial(`${data.name}-multiMat`, scene);
				...
				box
					.translate(Axis.X, element.from[0] + elementSize[0] / 2)
