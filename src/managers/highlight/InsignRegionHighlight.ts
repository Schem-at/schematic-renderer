...
export class InsignRegionHighlight implements Highlight {
	...
	activate(): void {
		...
		for (const box of this.entry.bounding_boxes) {
			...
			if (mesh) {
				...
				console.log(
					`[InsignRegionHighlight] Added mesh for ${this.regionId} at position:`,
					mesh.position,
					"box:",
					box
				);
		...
		console.log(
			`[InsignRegionHighlight] Activated region '${this.regionId}' with ${this.meshes.length} boxes and ${this.labels.length} labels`
		);
	}
	...
	private createThickEdges(width: number, height: number, depth: number): THREE.Group {
		...
		// Bottom edges
		group.add(
			createEdge(width, new THREE.Vector3(0, -hh, -hd), new THREE.Euler(0, 0, Math.PI / 2))
		);
		...
		// Bottom edges
		group.add(
			createEdge(depth, new THREE.Vector3(-hw, -hh, 0), new THREE.Euler(Math.PI / 2, 0, 0))
		);
