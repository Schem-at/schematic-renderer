...
export class ClickInteractionHandler implements Highlight {
	...
	private onClick = (event: MouseEvent) => {
		...
		if (intersects.length > 0) {
			...
			for (const schematic of schematics) {
				if (
					schematic.group === intersect.object.parent ||
			...
			if (intersect.face) {
				...
				const localFaceNormal = faceNormal
					.clone()
					.transformDirection(targetSchematic.group.matrixWorld)
					.normalize();
				...
				const epsilon = 0.001;
				if (
					Math.abs(localFaceNormal.x) > 0.9 &&
					Math.abs(fractional.x) < epsilon &&
					localFaceNormal.x > 0
				) {
					localPosition.x -= 1;
				}
				if (
					Math.abs(localFaceNormal.y) > 0.9 &&
					Math.abs(fractional.y) < epsilon &&
					localFaceNormal.y > 0
				) {
					localPosition.y -= 1;
				}
				if (
					Math.abs(localFaceNormal.z) > 0.9 &&
					Math.abs(fractional.z) < epsilon &&
					localFaceNormal.z > 0
				) {
					localPosition.z -= 1;
				}
